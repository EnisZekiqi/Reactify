...
const SecondNavbar = () => {
  ...
  useEffect(() => {
    ...
    const filtered = examples.filter((example) =>
      example.id.toLocaleLowerCase().includes(searchQuery.toLocaleLowerCase()),
    );
    ...
    <div>
      <div className="navbar2 flex justify-between fixed w-screen px-1 sm:px-4 ">
        ...
        <div className="flex flex-row-reverse items-center gap-4 text-sm font-normal mt-4">
          <a
            className="githubExample opacity-70 hover:opacity-100 bg-[#00d8ff] px-3.5 py-1.5 font-medium rounded-md text-black transition-all duration-300"
          ...
          <button
            ...
            className="drawerComponent bg-transparent  border border-[#343434] lg:border-none rounded-xl p-2 cursor-pointer text-[rgba(255,255,255,0.7) hover:text-white transition-all duration-300]"
          >
            {DrawerComponent ? (
              <IoMdClose className="w-[18px] h-[18px]" />
            ) : (
              <CiMenuBurger className="w-[18px] h-[18px]" />
            )}
          </button>
          {!DrawerComponent && (
            <button
              onClick={() => setOpenSearch(true)}
              className="bg-transparent  border border-[#343434] lg:border-none rounded-xl p-2 cursor-pointer text-[rgba(255,255,255,0.7)] hover:text-white transition-all duration-300"
            >
        ...
        <AnimatePresence mode="wait">
          {DrawerComponent && (
            ...
            >
              ...
              {examples
                .filter((example) => example.for === "in Text Animation")
                .map((example) => (
              ...
              <div className="border-t flex flex-col text-md font-medium w-[90%] ml-3 border-[#343434] pt-2 mt-6">
                ...
                <a href="https://github.com/EnisZekiqi" target="_blank">
                  Who made this?
                </a>
              </div>
            </motion.div>
