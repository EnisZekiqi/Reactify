...
const Ending = () => {
    return ( 
        <div className="app-bg2 h-screen">
    <div className="relative flex flex-col items-center justify-center mt-10 z-[90]">
      ...
      <motion.img initial={{
        ...
        transition: {
          ...
          alt="Title Background" className="absolute mask-t-from-10% inset-0 w-screen h-screen object-contain pointer-events-none select-none z-0  blur-md" />
      ...
      <motion.h1 initial={{
        ...
        transition: {
          ...
          }} className="relative font-medium text-2xl md:text-3xl lg:text-5xl text-white w-2/3  z-[10] text-center px-6 py-3">
        Delight the web with Reactify
                </motion.h1>
                <motion.button
                initial={{ opacity: 0 }}
                whileInView={{opacity:1,transition:{delay:1,duration:0.5}}}
                className="mt-4 md:px-6 md:py-3  sm:px-4 sm:py-2 px-2 py-2 w-full sm:w-fit bg-[#00d8ff] hover:bg-[#00adcc] text-black rounded-lg shadow font-medium" whileHover={{
                scale: 1.05
              }} whileTap={{
                scale: 0.95
              }}>
              Get Started
                ...
                whileInView={{ opacity: 1, transition: { delay: 1, duration: 0.5 } }}
                viewport={{once:true}}
                className="mt-[20%] flex items-center gap-3">Created with passion by <a href="https://eniszekiqidev.netlify.app/" target="_blank" className="text-[rgba(255,255,255,0.5)] hover:underline transition duration-300" >Enis Zekiqi</a></motion.p>
            </div>
            </div>
            </div>
         );
