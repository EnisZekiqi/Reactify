...
const FluidMenu = () => {
  ...
  const [clipOrigin, setClipOrigin] = useState<{ x: string; y: string }>({
    x: "50%",
    y: "50%",
  });
        ...
        <AnimatePresence>
          {menuOpen && (
            <motion.div
              key="mobileMenu"
              initial={{
                clipPath: 'circle(0px at $ {clipOrigin.x} $ {clipOrigin.y})',
              }}
              animate={{
                clipPath: 'circle(2000px at $ {clipOrigin.x} $ {clipOrigin.y})',
                transition: { duration: 0.6, ease: "easeInOut" },
              }}
              exit={{
                clipPath: 'circle(0px at $ {clipOrigin.x} $ {clipOrigin.y})',
