...
const filteredButtons = button3.filter(btn => btn !== 'Free' && btn !== 'Premium');
  ...
  <span
    style={{
      ...
      transition: 'color 1s easeInOut', zIndex:100
    }}
...
// Define types for mode and day
type Mode = "Free" | "Premium";
type DayOption = string; // you could tighten this based on known options if needed

const SliderButton: React.FC = () => {
  const [selectMode, setSelectMode] = useState<Mode>("Free");
  const [selectDay, setSelectDay] = useState<DayOption>("Annual");

  // Filter out the mode buttons from day options
  const filteredButtons: DayOption[] = button3.filter(
    (btn: string) => btn !== "Free" && btn !== "Premium"
  );
